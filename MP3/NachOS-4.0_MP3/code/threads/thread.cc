...
Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    this->setPredictedBurstTime(0);
    this->setRunTime(0);

    DEBUG(dbgSjf, "Thread(Forked) [" << this->getID() << "]'s RunTime is " 
                    << this->getRunTime() << ", PredictedBurstTime is " 
                    << this->getPredictedBurstTime() << endl);
...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) {
        ...
        int elapsedTicks = kernel->stats->totalTicks - this->getLastUpdateTime();
        this->setRunTime(this->getRunTime() + elapsedTicks);
        this->setLastUpdateTime(kernel->stats->totalTicks);

        DEBUG(dbgSjf, "[E] Tick [" 
            << kernel->stats->totalTicks << "]: Thread [" << nextThread->getID()
            << "] is now selected for execution, thread [" << kernel->currentThread->getID()
            << "] is preempted, and it has executed [" << kernel->currentThread->getRunTime()
            << "] ticks");

        DEBUG(dbgSjf, "Thread(before Yield) [" << this->getID() << "]'s RunTime is " 
                    << this->getRunTime() << ", PredictedBurstTime is " 
                    << this->getPredictedBurstTime());

        int PrevBurstTime = kernel->currentThread->getPredictedBurstTime();
        int diff = kernel->currentThread->getPredictedBurstTime() - kernel->currentThread->getRunTime();
        int temp = 0.5 * kernel->currentThread->getRunTime() + 0.5 * kernel->currentThread->getPredictedBurstTime();
        int ApproximateBurstTime = diff<0 ? temp : diff;
        ...
        if (kernel->currentThread->getID() > 0) {
            DEBUG(dbgSjf, "[C] Tick [" << kernel->stats->totalTicks << "]: Thread [" 
                << kernel->currentThread->getID() << "] update approximate burst time, from [" 
                << PrevBurstTime << "], add [" 
                << kernel->currentThread->getPredictedBurstTime() - PrevBurstTime 
                << "], to [" << ApproximateBurstTime << "]");
        }

        DEBUG(dbgSjf, "Thread(after Yield) [" << this->getID() << "]'s RunTime is " 
                   << this->getRunTime() << ", PredictedBurstTime is " 
                   << this->getPredictedBurstTime() << endl);
...
Thread::Sleep (bool finishing)
{
    ...
    int elapsedTicks = kernel->stats->totalTicks - kernel->currentThread->getLastUpdateTime();
    kernel->currentThread->setRunTime(kernel->currentThread->getRunTime() + elapsedTicks);
    kernel->currentThread->setLastUpdateTime(kernel->stats->totalTicks);
    
    DEBUG(dbgSjf, "Thread(before BLOCKED) [" << kernel->currentThread->getID() << "]'s RunTime is " 
                   << kernel->currentThread->getRunTime() << ", PredictedBurstTime is " 
    ...
    DEBUG(dbgSjf, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getID()
        << "] is now selected for execution, thread [" << kernel->currentThread->getID()
        << "] is starts IO, and it has executed [" << kernel->currentThread->getRunTime()
        << "] ticks");
    // returns when it's time for us to run

    kernel->currentThread->setRunTime(0);

    DEBUG(dbgSjf, "Thread(after BLOCKED) [" << kernel->currentThread->getID() << "]'s RunTime is " 
                   << kernel->currentThread->getRunTime() << ", PredictedBurstTime is " 
                   << kernel->currentThread->getPredictedBurstTime() << endl);
